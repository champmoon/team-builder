...
reset_password: Docs = {
    ...
    P.S.
        Этот запрос сработает только 1 раз.
    """,
    "responses": {
        201: {
        ...
        404: {
            "description": "Пользователь с таким `email` нету",
...
confirm_password: Docs = {
    "summary": "Подтверждение email для смены пароля.",
    "description": """
    ```
    Request Body:
    ...
    P.S.:
        ...
        При смены пароля на почту отправляется ссылка
        с confirm_token, который хранит информацию про пользователя.

        Ссылка активна 5 минут, после этого она протухает.

        При успешном ответе возвращается почта,
        которая автоматически подставляется в дальнейшую форму смены пароля.
    """,
    "responses": {
        200: {
...
send_confirm_password: Docs = {
    "summary": "Отправка подтверждающего email для сменя пароля.",
    "description": """
    ```
    Request Body:
        email - почта.
    ...
    P.S.:
        Перед началом смены пароля пользователю необходимо подтвердить свою почту.
